    ...
    def _parse_metadata(meta_str: Optional[str]) -> Dict[str, str]:
        ...
        for pair in pairs:
            if '=' in pair:
                ...
                # E.g., handling colors without 'color=' prefix if they exist, 
                # though subject examples use 'color=red'
                meta_dict[pair] = "true" 
        return meta_dict

    @staticmethod
    def parse_file(filepath: str) -> Network:
        network = Network()
        nb_drones_found = False

        # Regex patterns to enforce syntax and forbidden characters (no spaces/dashes in names)
        # ^(start_hub|end_hub|hub):\s+([^\s\-]+)\s+(\d+)\s+(\d+)(?:\s+\[(.*?)\])?$
        zone_pattern = re.compile(r"^(start_hub|end_hub|hub):\s+([^\s\-]+)\s+(-?\d+)\s+(-?\d+)(?:\s+\[(.*?)\])?$")
        # ^connection:\s+([^\s\-]+)-([^\s\-]+)(?:\s+\[(.*?)\])?$
        conn_pattern = re.compile(r"^connection:\s+([^\s\-]+)-([^\s\-]+)(?:\s+\[(.*?)\])?$")
                ...
                for line_num, line in enumerate(f, 1):
                    ...
                    try:
                        ...
                        # 3. Parse Zones
                            ...
                            if not match:
                                raise ValueError("Invalid zone format or zone name contains dashes/spaces.")
                            ...
                            try:
                                ...
                                raise ValueError(f"Invalid zone type '{z_type_str}'. Must be normal, blocked, restricted, or priority.")
                        ...
                        # 4. Parse Connections
                            ...
                            if not match:
                                raise ValueError("Invalid connection format. Check for dashes in names.")
                            
                            name1, name2, meta_str = match.groups()
                            
                            if name1 not in network.zones or name2 not in network.zones:
                                raise ValueError(f"Connection links unknown zones: {name1}, {name2}.")
        ...
        for drone in network.drones:
            drone.current_zone = network.start_hub
